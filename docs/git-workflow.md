# Git 工作流程与分支管理

**版本**: 1.0.0
**更新时间**: 2026-01-15
**状态**: Production Ready

---

## 🌳 分支策略

### 分支模型

采用 **Speckit 分支策略** - 基于数字前缀的功能分支模型:

```
main (生产分支)
  ├── 001-lark-service-core (功能分支 - Phase 1-2)
  ├── 002-messaging-service (功能分支 - Phase 3)
  ├── 003-document-integration (功能分支 - Phase 4)
  ├── release/v0.2.0 (发布分支)
  └── hotfix/v0.1.1-security (热修复分支)
```

### 分支类型与规范

| 分支类型 | 命名规范 | 生命周期 | 合并目标 | 说明 |
|---------|---------|---------|---------|------|
| **main** | `main` | 永久 | - | 生产代码 |
| **功能分支** | `NNN-<描述>` | 临时 | main | Speckit 管理 |
| **release** | `release/v<版本>` | 临时 | main | 发布准备 |
| **hotfix** | `hotfix/v<版本>-<描述>` | 临时 | main | 紧急修复 |

**注意**:
- `NNN` 是3位数字前缀 (如 `001`, `002`, `003`)
- 功能分支由 `/speckit.specify` 命令自动创建
- 每个功能分支对应 `specs/NNN-<描述>/` 目录

### 分支详细说明

#### 1. main (主分支)

**用途**: 生产就绪代码,每个提交都是一个可发布版本

**保护规则**:
- ✅ 禁止直接推送
- ✅ 必须通过 PR
- ✅ 至少 1 人代码审查
- ✅ CI 必须通过
- ✅ 必须签名提交 (可选)

**标签策略**:
```bash
# 每次合并到 main 都打标签
git tag -a v0.1.0 -m "Release v0.1.0: Phase 1-2 完成"
git push origin v0.1.0
```

#### 2. NNN-<描述> (Speckit 功能分支)

**命名规范**: `NNN-<short-description>`
- `NNN`: 3位递增数字 (001, 002, 003...)
- `<short-description>`: 2-4 个关键词,连字符分隔

**命名示例**:
```
001-lark-service-core
002-messaging-service
003-document-integration
004-user-auth-enhancement
```

**创建方式** (使用 Speckit):
```bash
# 方式 1: 自动生成分支名
/speckit.specify "Implement messaging service for group chats"
# 创建: 002-messaging-service

# 方式 2: 指定短名称
/speckit.specify "Add OAuth2 authentication" --short-name "user-auth"
# 创建: 003-user-auth

# 方式 3: 指定分支号
/speckit.specify "Performance optimization" --number 5
# 创建: 005-performance-optimization
```

**分支结构**:
```
项目根目录/
  ├── specs/
  │   ├── 001-lark-service-core/
  │   │   ├── spec.md            (需求规范)
  │   │   ├── plan.md            (实施计划)
  │   │   ├── tasks.md           (任务清单)
  │   │   └── checklists/        (检查清单)
  │   ├── 002-messaging-service/
  │   └── 003-document-integration/
```

**工作流程**:
```bash
# 1. 创建功能分支 (Speckit 自动创建)
/speckit.specify "Implement messaging service"
# → 创建分支: 002-messaging-service
# → 创建目录: specs/002-messaging-service/
# → 创建文件: spec.md, plan.md, tasks.md

# 2. 编写规范和计划
/speckit.plan     # 生成实施计划
/speckit.tasks    # 生成任务清单

# 3. 开发功能
git add .
git commit -m "feat(messaging): 实现消息发送接口"

# 4. 定期同步 main
git fetch origin main
git rebase origin/main

# 5. 推送到远程
git push -u origin 002-messaging-service

# 6. 创建 PR: 002-messaging-service → main
# (通过 GitHub 界面)

# 7. 合并后删除分支
git checkout main
git pull
git branch -d 002-messaging-service
```

**多分支共享同一 Spec**:

Speckit 支持多个分支共享同一个 spec 目录:

```bash
# 主功能分支
001-lark-service-core

# 基于同一 spec 的其他分支
001-fix-token-bug
001-add-metrics
001-refactor-storage

# 它们都使用 specs/001-lark-service-core/ 目录
```

#### 3. release/* (发布分支)

**用途**: 准备新版本发布,仅允许 bug 修复和版本号更新

**命名规范**: `release/v<版本号>`

**工作流程**:
```bash
# 1. 从 main 创建发布分支
git checkout main
git pull origin main
git checkout -b release/v0.2.0

# 2. 更新版本号
echo "0.2.0" > VERSION
git commit -am "chore: bump version to 0.2.0"

# 3. 最后的 bug 修复
git commit -m "fix: 修复发布前发现的问题"

# 4. 合并到 main 并打标签
git checkout main
git merge --no-ff release/v0.2.0
git tag -a v0.2.0 -m "Release v0.2.0"
git push origin main --tags

# 5. 删除发布分支
git branch -d release/v0.2.0
git push origin --delete release/v0.2.0
```

#### 4. hotfix/* (热修复分支)

**用途**: 紧急修复生产环境问题

**命名规范**: `hotfix/v<版本号>-<描述>`

**工作流程**:
```bash
# 1. 从 main 创建热修复分支
git checkout main
git pull origin main
git checkout -b hotfix/v0.1.1-security-fix

# 2. 修复问题
git commit -m "fix(security): 修复 SQL 注入漏洞"

# 3. 合并到 main 并打补丁标签
git checkout main
git merge --no-ff hotfix/v0.1.1-security-fix
git tag -a v0.1.1 -m "Hotfix v0.1.1: 安全修复"
git push origin main --tags

# 4. 删除热修复分支
git branch -d hotfix/v0.1.1-security-fix
git push origin --delete hotfix/v0.1.1-security-fix
```

---

## 🏷️ 标签规范 (CHK115)

### 语义化版本 (SemVer)

格式: `v<主版本>.<次版本>.<补丁版本>`

```
v0.1.0 - Phase 1-2 完成 (初始版本)
v0.2.0 - Phase 3 完成 (新增消息功能)
v0.2.1 - 修复 Token 刷新 Bug (补丁)
v0.3.0 - Phase 4 完成 (新增文档功能)
v1.0.0 - 生产就绪 (稳定版本)
```

### 版本号规则

| 版本类型 | 何时递增 | 示例 |
|---------|---------|------|
| **主版本** | 不兼容的 API 变更 | 0.x.x → 1.0.0 |
| **次版本** | 向后兼容的功能新增 | 0.1.x → 0.2.0 |
| **补丁版本** | 向后兼容的 bug 修复 | 0.1.0 → 0.1.1 |

### 标签创建命令

```bash
# 轻量标签 (不推荐)
git tag v0.1.0

# 附注标签 (推荐)
git tag -a v0.1.0 -m "Release v0.1.0: Phase 1-2 完成

- ✅ 配置管理系统
- ✅ Token 管理池
- ✅ SQLAlchemy 2.0 升级
- ✅ 77.33% 测试覆盖率
- ✅ 安全合规 (FR-077~095)
"

# 推送标签
git push origin v0.1.0

# 推送所有标签
git push origin --tags
```

### Phase 标签策略

| Phase | 分支 | 标签 | 说明 |
|-------|------|------|------|
| Phase 1-2 | `001-lark-service-core` | v0.1.0 | 基础设施 + Token 管理 |
| Phase 3 | `002-messaging-service` | v0.2.0 | 消息服务 |
| Phase 4 | `003-document-integration` | v0.3.0 | 文档+通讯录 |
| Phase 5 | `004-apaas-features` | v0.4.0 | aPaaS 功能 |
| Stable | - | v1.0.0 | 生产就绪 |

**分支与标签对应关系**:
```bash
# Phase 1-2 完成
git checkout main
git merge --no-ff 001-lark-service-core
git tag -a v0.1.0 -m "Release v0.1.0: Phase 1-2 完成"

# Phase 3 完成
git merge --no-ff 002-messaging-service
git tag -a v0.2.0 -m "Release v0.2.0: Phase 3 消息服务"

# 以此类推...
```

---

## 📦 提交规范

### Conventional Commits

格式: `<类型>(<范围>): <描述>`

```
feat(messaging): 实现消息发送接口
fix(token): 修复 Token 刷新逻辑
docs(readme): 更新安装说明
style(core): 格式化代码
refactor(storage): 重构数据库连接池
test(cli): 添加 CLI 单元测试
chore(deps): 升级 SQLAlchemy 到 2.0.25
```

### 提交类型

| 类型 | 说明 | 示例 |
|------|------|------|
| **feat** | 新功能 | `feat(auth): 添加 OAuth 2.0 支持` |
| **fix** | Bug 修复 | `fix(token): 修复过期判断逻辑` |
| **docs** | 文档变更 | `docs(api): 更新 API 文档` |
| **style** | 代码格式 | `style: 运行 ruff format` |
| **refactor** | 重构 | `refactor(db): 优化查询性能` |
| **test** | 测试 | `test(unit): 添加 CredentialPool 测试` |
| **chore** | 构建/工具 | `chore(ci): 添加 GitHub Actions` |
| **perf** | 性能优化 | `perf(cache): 使用 Redis 缓存` |

### 提交描述规范

**好的提交**:
```bash
✅ feat(messaging): 实现群消息发送功能

- 支持文本、图片、卡片消息
- 实现批量发送接口
- 添加消息发送限流保护

Closes #42
```

**差的提交**:
```bash
❌ update
❌ fix bug
❌ 修改了一些文件
```

---

## 🔄 Phase 过渡流程 (CHK117)

### Phase 完成标准

#### Phase 1 → Phase 2 过渡标准 ⭐

**必须满足 (P1 - Blocker)**:
- [ ] **Blocker Gap = 0** - 无关键阻塞问题
- [ ] **Major Gap ≤ 3** - 重大问题已基本解决
- [ ] **测试覆盖率 ≥ 60%** - 核心模块测试充分
- [ ] **性能基线已建立** - Token刷新 < 100ms, API吞吐量 ≥ 100 req/s
- [ ] **CI 全部通过** - 代码质量、安全扫描无错误
- [ ] **Phase 1-2 过渡标准已定义** - 本文档更新完成

**应当满足 (P2 - Major)**:
- [ ] 启动失败回滚策略已定义
- [ ] 故障排查文档已创建
- [ ] 构建时间基线已建立

**可选满足 (P3 - Minor)**:
- [ ] 边界条件覆盖完善
- [ ] 技术债务管理机制建立

**验收决策**:
- ✅ **通过**: 所有 P1 条件满足 + P2 条件 ≥ 80%
- ⚠️ **有条件通过**: 所有 P1 条件满足 + P2 条件 ≥ 50%
- ❌ **不通过**: P1 条件 < 100%

**当前状态**:
- 综合评分: 82%
- Blocker: 0 项 ✅
- Major Gap: 8 项 ⚠️
- 决策: **有条件通过** (完成 P1 后可进入 Phase 2)

---

#### Phase 1-2 完成检查清单 (历史参考)

**功能完整性**:
- [x] 所有 T001-T015 任务完成
- [x] 所有验收标准达标
- [x] 无 Blocker 和 Major Gap

**质量标准**:
- [x] 测试覆盖率 ≥ 75%
- [x] Mypy 覆盖率 ≥ 99%
- [x] 0 个 Ruff 错误
- [x] CI 全部通过

**文档完整性**:
- [x] README 完善
- [x] API 文档完整
- [x] 架构文档更新
- [x] 部署文档完善

**安全合规**:
- [x] FR-077~095 全部实现
- [x] 依赖安全扫描通过
- [x] 容器安全扫描通过

#### Phase 过渡流程

```
Phase N 开发 → 自测试 → Code Review → CI/CD → Phase 验收 → 打标签 → 启动 Phase N+1
```

**详细步骤**:

1. **自测试** (开发者)
   ```bash
   # 切换到功能分支
   git checkout 001-lark-service-core

   # 运行测试
   pytest tests/ --cov=src

   # 检查类型
   mypy src/

   # 检查代码风格
   ruff check src/

   # 检查文档
   pydocstyle src/

   # 运行 Speckit 检查清单
   /speckit.checklist
   ```

2. **代码审查** (团队)
   - 创建 PR: `001-lark-service-core → main`
   - 至少 1 人审查
   - 解决所有评论
   - 确保 `specs/001-lark-service-core/` 文档完整

3. **CI/CD 验证** (自动)
   - 测试通过
   - 覆盖率达标
   - 代码质量检查通过
   - 安全扫描通过

4. **Phase 验收** (技术负责人)
   - 运行 Speckit 检查清单
   - 验证 `specs/001-lark-service-core/` 交付物
   - 检查 `spec.md`, `plan.md`, `tasks.md` 完成度
   - 更新文档

5. **打标签发布** (维护者)
   ```bash
   git checkout main
   git pull origin main
   git merge --no-ff 001-lark-service-core
   git tag -a v0.1.0 -m "Release v0.1.0: Phase 1-2 完成

   - ✅ 配置管理系统
   - ✅ Token 管理池
   - ✅ 测试覆盖率 77.33%
   - ✅ 安全合规 (FR-077~095)

   Spec: specs/001-lark-service-core/
   "
   git push origin main --tags

   # 可选: 删除已合并的功能分支
   git branch -d 001-lark-service-core
   git push origin --delete 001-lark-service-core
   ```

6. **启动下一 Phase**
   ```bash
   # 使用 Speckit 创建下一个功能分支
   /speckit.specify "Implement messaging service for group chats"
   # → 创建: 002-messaging-service
   # → 创建: specs/002-messaging-service/

   # 编写规范
   /speckit.plan
   /speckit.tasks

   # 分配任务给团队
   ```

### Phase 过渡条件矩阵

| 从 Phase | 到 Phase | 必要条件 | 建议条件 | 验收标准 |
|---------|---------|---------|---------|---------|
| **Phase 1** | **Phase 2** | • Blocker Gap = 0<br>• Major Gap ≤ 3<br>• 测试覆盖率 ≥ 60%<br>• 性能基线已建立<br>• CI 全部通过 | • 测试覆盖率 ≥ 80%<br>• 故障排查文档完善<br>• 构建时间基线建立 | **有条件通过**<br>评分: 82%<br>P1 完成后可进入 |
| Phase 2 | Phase 3 | • Token 管理功能完整<br>• 核心 API 稳定<br>• 集成测试通过 | • 性能测试通过<br>• 文档完善 | 待定义 |
| Phase 3 | Phase 4 | • 消息发送功能完整<br>• 消息接收功能完整 | • 性能测试通过<br>• 用户反馈收集 | 待定义 |
| Phase 4 | Phase 5 | • 文档+通讯录集成<br>• 所有 API 测试通过 | • 负载测试通过<br>• 安全审计完成 | 待定义 |
| Phase 5 | v1.0 | • 所有功能完成<br>• 生产环境验证<br>• 性能达标 | • 负载测试通过<br>• 安全合规认证 | 待定义 |

**说明**:
- **必要条件**: 不满足则无法进入下一 Phase
- **建议条件**: 建议满足,但不阻塞 Phase 过渡
- **验收标准**: 基于 `/speckit.checklist` 评估结果

---

## 📋 代码审查流程 (Code Review)

### 审查清单

#### 功能审查
- [ ] 功能实现符合需求
- [ ] 边界条件处理正确
- [ ] 错误处理完善
- [ ] 测试覆盖充分

#### 代码质量
- [ ] 命名清晰易懂
- [ ] 函数/类职责单一
- [ ] 避免代码重复
- [ ] 复杂度可接受
- [ ] 注释清晰充分

#### 类型安全
- [ ] 类型注解完整
- [ ] Mypy 检查通过
- [ ] 无 type: ignore (或有正当理由)

#### 安全性
- [ ] 无硬编码密钥
- [ ] 输入验证完善
- [ ] SQL 注入防护
- [ ] 敏感信息脱敏

#### 文档
- [ ] Docstring 完整 (Google Style)
- [ ] README 更新
- [ ] CHANGELOG 更新

### 审查流程

```
PR 创建 → 自动检查 → 人工审查 → 反馈修改 → 审批合并
```

**时效要求**:
- 小 PR (< 200 行): 24 小时内审查
- 中 PR (200-500 行): 48 小时内审查
- 大 PR (> 500 行): 72 小时内审查 (建议拆分)

### PR 模板

**`.github/PULL_REQUEST_TEMPLATE.md`**:

```markdown
## 变更类型
- [ ] Feature (新功能)
- [ ] Fix (Bug 修复)
- [ ] Docs (文档)
- [ ] Refactor (重构)
- [ ] Test (测试)

## 分支信息
**功能分支**: `XXX-<描述>`
**Spec 目录**: `specs/XXX-<描述>/`

## 变更描述
[简要描述此 PR 的目的和内容]

## 相关 Issue
Closes #<issue_number>

## Speckit 文档
- [ ] `spec.md` - 需求规范已完成
- [ ] `plan.md` - 实施计划已完成
- [ ] `tasks.md` - 任务清单已完成
- [ ] `checklists/` - 检查清单已验证

## 测试
- [ ] 单元测试已添加/更新
- [ ] 集成测试已添加/更新
- [ ] 手动测试已完成
- [ ] 测试覆盖率 ≥ 75%

## 代码质量
- [ ] Ruff 检查通过 (0 错误)
- [ ] Mypy 检查通过 (99%+ 覆盖率)
- [ ] 所有 Docstring 已添加 (Google Style)
- [ ] 代码符合项目规范

## 文档
- [ ] README 已更新 (如需要)
- [ ] API 文档已更新 (如需要)
- [ ] CHANGELOG 已更新

## 截图 (如适用)
[添加截图]

## 额外说明
[其他需要审查者注意的内容]
```

---

## 📦 交付物归档策略 (CHK116)

### 归档时机

| 事件 | 归档内容 | 位置 |
|------|---------|------|
| **Phase 完成** | 代码快照 + 文档 | Git Tag |
| **版本发布** | Release 包 + Changelog | GitHub Releases |
| **重大里程碑** | 完整项目备份 | 外部存储 |

### 归档内容

#### 1. 代码归档

```bash
# Git Tag (已包含代码快照)
git tag -a v0.1.0 -m "Phase 1-2 完成"

# 创建归档包
git archive --format=tar.gz --prefix=lark-service-v0.1.0/ v0.1.0 > lark-service-v0.1.0.tar.gz
```

#### 2. 文档归档

**归档结构**:
```
archives/
  └── v0.1.0/
      ├── source-code.tar.gz          # 源代码
      ├── docs/                       # 文档快照
      ├── specs/                      # 需求规范
      ├── test-reports/               # 测试报告
      ├── coverage-report.html        # 覆盖率报告
      ├── security-scan.json          # 安全扫描结果
      └── CHANGELOG.md                # 变更日志
```

#### 3. Release Notes

**示例**:
```markdown
# Release v0.1.0 - Phase 1-2 完成

**发布日期**: 2026-01-15
**标签**: v0.1.0

## ✨ 新功能

- ✅ 配置管理系统 (SQLite + 加密)
- ✅ Token 管理池 (自动刷新 + 持久化)
- ✅ PostgreSQL Token 存储
- ✅ RabbitMQ 消息队列集成
- ✅ CLI 工具 (lark-service-cli)

## 🔧 改进

- SQLAlchemy 升级到 2.0 (现代语法)
- Mypy 覆盖率达到 99.8%
- 测试覆盖率达到 77.33%
- 安全合规 (FR-077~095)

## 📊 质量指标

- 测试: 140/140 通过
- 覆盖率: 77.33%
- Mypy: 99.8%
- Ruff: 0 错误

## 📚 文档

- 17 个文档 (8,515 行)
- 129 个需求条目
- 完整的 API 文档

## 🔗 资源

- [安装指南](docs/quickstart.md)
- [API 文档](docs/api.md)
- [架构设计](docs/architecture.md)

## 📦 下载

- Source code (tar.gz)
- Source code (zip)
```

### GitHub Releases 创建

```bash
# 使用 GitHub CLI
gh release create v0.1.0 \
  --title "Release v0.1.0 - Phase 1-2 完成" \
  --notes-file RELEASE_NOTES.md \
  lark-service-v0.1.0.tar.gz
```

---

## 🔧 常用命令

### Speckit 命令

```bash
# 创建新功能分支和 spec 目录
/speckit.specify "Feature description"
/speckit.specify "Feature" --short-name "short-name"
/speckit.specify "Feature" --number 5

# 生成实施计划
/speckit.plan

# 生成任务清单
/speckit.tasks

# 运行检查清单
/speckit.checklist

# 分析需求
/speckit.analyze

# 澄清需求
/speckit.clarify
```

### 分支管理

```bash
# 查看所有分支
git branch -a

# 创建功能分支 (推荐使用 Speckit)
/speckit.specify "New feature description"

# 切换分支
git checkout 001-lark-service-core

# 删除本地分支
git branch -d 001-old-feature

# 删除远程分支
git push origin --delete 001-old-feature

# 查看分支历史
git log --graph --oneline --all

# 查看当前分支对应的 spec 目录
ls -la specs/$(git branch --show-current)/
```

### 标签管理

```bash
# 查看所有标签
git tag -l

# 查看标签详情
git show v0.1.0

# 删除本地标签
git tag -d v0.1.0

# 删除远程标签
git push origin --delete v0.1.0

# 检出特定标签
git checkout v0.1.0
```

### 同步更新

```bash
# 同步远程分支
git fetch origin

# 拉取并变基
git pull --rebase origin develop

# 推送本地分支
git push -u origin feature/my-feature
```

---

## ✅ 流程检查清单

### Git 工作流
- [ ] 分支策略已定义
- [ ] 分支命名规范已遵守
- [ ] 提交信息符合规范
- [ ] 代码已推送到远程

### 标签管理
- [ ] 版本号符合 SemVer
- [ ] 标签信息完整
- [ ] 标签已推送到远程

### Phase 过渡
- [ ] 完成标准已验证
- [ ] 检查清单已完成
- [ ] 文档已更新
- [ ] 标签已创建

### 代码审查
- [ ] PR 已创建
- [ ] 审查意见已处理
- [ ] CI 检查已通过
- [ ] 至少 1 人审批

### 交付归档
- [ ] Release Notes 已创建
- [ ] GitHub Release 已发布
- [ ] 归档包已生成
- [ ] 文档已归档

---

**维护者**: Lark Service Team
**参考**: [git-workflow.md](./git-workflow.md)
