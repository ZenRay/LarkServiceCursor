...
| **Phase 3** | WebSocket 客户端 (T011-T024) | ⏳ 待开始 | - | - |
...
## 🚀 下一步: Phase 3 - WebSocket 客户端

### 任务范围 (T011-T024, 14 tasks)

**目标**: 实现 WebSocket 长连接客户端,接收飞书事件

#### 核心任务
1. **T011-T013**: `LarkWebSocketClient` 类实现
   - 连接管理、事件分发、断线重连
2. **T014-T016**: 事件处理器注册
   - `P2CardActionTrigger` 事件处理
3. **T017-T019**: 连接状态管理
   - 心跳、重连、降级到 HTTP
4. **T020-T024**: 测试和集成
   - 单元测试、集成测试、契约测试
...
#### 关键文件
- `src/lark_service/events/websocket_client.py` (新建)
- `tests/unit/events/test_websocket_client.py` (新建)
- `tests/integration/test_websocket_integration.py` (新建)
...
```bash
# 在新的 chat 中运行
/speckit.implement 执行 Phase 3 任务 (T011-T024)
```
...
**状态**: ✅ Phase 2 完成,所有交付物就绪,准备开始 Phase 3
**下一步**: 实施 WebSocket 客户端 (T011-T024)
**预计完成**: Phase 3 需 2.5-3.5 天,MVP (Phase 1-4) 需 6-8 天
