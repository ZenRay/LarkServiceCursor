...
- ⏰ **定时任务**: APScheduler 集成,支持用户同步、Token 监控等自动化任务 (v0.5.0+) 🆕
- 🐳 **Docker 容器化**: 完整的 Docker Compose 生产环境支持 (v0.5.0+) 🆕
...
- 🔧 **[部署与运维](docs/deployment/)** - 生产环境部署和运维
  - [生产环境部署指南](docs/deployment/PRODUCTION_DEPLOYMENT.md) - 完整部署流程 🆕
  - [部署指南](docs/deployment.md) - 基础部署
  - [监控指南](docs/monitoring.md) - Prometheus & Grafana
- 🏗️ **[架构文档](docs/architecture/)**
  - [Token 刷新机制](docs/architecture/token-refresh-mechanism.md) - 三种 Token 类型详解 🆕
- 🎯 **[功能特性](docs/features/)**
  - [Token 监控](docs/features/token-monitoring.md) - Token 过期监控和通知 🆕
- ✅ **[测试与验证](docs/testing/)**
  - [真实飞书环境集成验证](docs/testing/INTEGRATION_VERIFICATION.md) - 完整验证指南 🆕
  - [测试指南](docs/TESTING-GUIDE.md)
- 📝 **[发布说明](docs/releases/)**
  - [v0.5.0 发布说明](docs/releases/v0.5.0.md) - 生产环境完整集成 🎉 🆕
...
### v0.5.0 - 生产环境完整集成 (2026-01-22) 🎉

**重大更新**: 项目现已**生产就绪(Production Ready)**

#### 🚀 新功能
- ✅ **Docker 容器化**: 完整的 Docker Compose 生产环境支持
- ✅ **APScheduler 定时任务**: 用户同步、Token 监控、清理等自动化任务
- ✅ **Token 监控优化**: 支持 3 种 Token 类型,智能过期监控和多级通知
- ✅ **Prometheus + Grafana**: 30+ 业务指标,3 个预配置仪表板,4 个告警规则
- ✅ **健康检查**: `/health` 端点,支持 Kubernetes 探针
...
#### ⚠️ Breaking Changes
- Token 监控逻辑调整: 仅监控 User Token 的 refresh_token
- 新增必需环境变量: `LARK_CONFIG_ENCRYPTION_KEY`

**详见**: [v0.5.0 完整发布说明](docs/releases/v0.5.0.md)

### v0.4.0 - 定时任务与 Token UX 优化 (2026-01-21)
- APScheduler 定时任务框架
- Token 过期 UX 优化
- Grafana 仪表板和 Prometheus 告警

### v0.3.0 - 多应用管理优化 (2026-01-20)
- 智能应用管理,5 层 app_id 优先级
- 应用配置持久化
- 完整的单元测试覆盖

### v0.2.0 - WebSocket 用户授权 (2026-01-19)
- 交互式卡片 OAuth 授权
- user_access_token 支持
- aPaaS AI 能力集成

### v0.1.0 - 核心组件 (2026-01-15)
- Token 自动管理
- 基础 API 封装 (消息、通讯录、云文档)
- SQLAlchemy 2.0 存储
